...
// console.log(props.menuItems[0])

const incrimentItem = (item) => {
   const incriment =  ++item.quantity;
    item.quantity = incriment;
// console.log(props.menuItems[props.menuItems.length - 1].quantity = incriment)   
            ...
            <div v-for="item in menuItems" :key="item.id" class="px-5">
                <div class="flex justify-between items-center border-b pb-5 pt-5">
                    <div class="flex">
                        ...
                        <div class="flex flex-col pl-3">
                            ...
                            <p class="text-base font-mono font-medium text-gray-600">
                               <!-- TK: {{ item.caloriesPerServing * item.quantity}} -->
                              {{ perItemPrice(item) }}
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="flex justify-between items-center border border-pink-400 rounded-2xl px-3 w-24">
                        <button @click="decrimentItem(item)" class="text-pink-600 text-xl font-bold ">
                            -
                        </button>
